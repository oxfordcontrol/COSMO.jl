function calculate_residuals(ws::COSMO.Workspace, settings::COSMO.Settings, IGNORESCALING_FLAG::Bool = false)
...
function max_res_component_norm(ws::COSMO.Workspace, settings::COSMO.Settings, IGNORESCALING_FLAG::Bool = false)
	if (settings.scaling != 0 && !IGNORESCALING_FLAG)
...
function has_converged(ws::COSMO.Workspace, settings::COSMO.Settings, r_prim::Float64, r_dual::Float64)
	max_norm_prim, max_norm_dual = max_res_component_norm(ws, settings)
