...
function print_header(ws::COSMO.Workspace)
	...
	for (iii, set) in enumerate(sort(ws.p.C.sets, by = x -> -x.dim))
		...
		if isa(set, PsdCone) || isa(set, PsdConeTriangle) || isa(set, PsdConeTriangleLanczos)
			set_dim = string(set.n, "x", set.n)
		else
	...
	println("Iter:\tObjective:\tPrimal Res:\tDual Res:\tRho:\t\tTime(s):\t Pr. Dim \t Pr. It.")
	flush(stdout)
	nothing
end
...
function print_iteration(ws::COSMO.Workspace, iter::Int64, cost::Float64, r_prim::Float64, r_dual::Float64, running_time::Float64)
	settings = ws.settings
	lanczos_iterations, exact_projections = 0, 0
		...
		if mod(iter, settings.check_termination) == 0
			...
			for set in ws.p.C.sets
				if length(lanczos_dims) < 3 && isa(set, COSMO.PsdConeTriangleLanczos)
					...
					lanczos_iterations += set.lanczos_iterations
					set.lanczos_iterations = 0
			...
			@printf("%d\t%.4e\t%.4e\t%.4e\t%.4e\t%.2e\t%s\t%s\n", iter, cost, r_prim, r_dual, ws.ρ, running_time, lanczos_dims, string(lanczos_iterations, "/", exact_projections))
		else
